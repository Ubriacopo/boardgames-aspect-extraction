...
class ABAEDataset(BaseBoardgameDataset):
    ...
    def process_dataset(self, dataset: DataFrame | list):
        ...
        with_lost_information = self.dataset.map(lambda x: len(x) > self.max_seq_length).sum()

        with_lost_information > 0 and print(
            f"We loose information on {with_lost_information}({with_lost_information / len(self.dataset) * 100}% of ds)."
        )
...
class PositiveNegativeABAEDataset(ABAEDataset):
    ...
    def __getitem__(self, index):
        ...
        return [sample, negative_samples], None
